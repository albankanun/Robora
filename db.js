...
// ---- create a reservation (from the chat assistant, Rina) -------------------
function createPreorder({ name, email, phone, items, language, note }) {
  ...
  retailTotal = +retailTotal.toFixed(2); preTotal = +preTotal.toFixed(2);

  const db = load();
  const order = {
    id: newId("PRE"),
    source: "rina",                 // <-- chat orders tagged "rina"
    name, email, phone,
    language: language || "English", note: note || "",
    lines, retail_total: retailTotal, preorder_total: preTotal,
    you_save: +(retailTotal - preTotal).toFixed(2), currency: "EUR",
    status: "new", createdAt: new Date().toISOString(),
  };
  db.preorders.push(order); save(db);
  return {
    success: true, order_id: order.id,
    summary: lines.map(l => `${l.name} × ${l.quantity} = €${l.line_total.toFixed(2)}`).join("; "),
    total: preTotal,
    message: `Reservation recorded. The team will email ${email} to confirm and arrange delivery by ${POLICIES.deliveryWindow}. No payment is taken now.`,
    _event: { type: "preorder", order },
  };
}

// ---- record a WEBSITE order (from robora.eu cart) ---------------------------
function recordWebOrder({ name, email, phone, order, total }) {
  const db = load();
  const rec = {
    id: newId("WEB"),
    source: "web",                  // <-- website orders tagged "web"
    name, email, phone: phone || "",
    raw_order: order || "",         // itemised text from the website cart
    lines: [],
    retail_total: Number(total) || 0,
    preorder_total: Number(total) || 0,
    you_save: 0, currency: "EUR",
    status: "new", createdAt: new Date().toISOString(),
  };
  db.preorders.push(rec); save(db);
  return rec;
}
...
module.exports = { checkPrice, createPreorder, recordWebOrder, takeMessage, snapshot, preof, saveof };
